...
import { z } from "zod";
import { useCreateSellerProduct } from "@/features/seller/queries/use-seller";
...
export default function NewProductPage() {
  ...
  const {
    ...
    defaultValues: { isActive: true, stock: 0, featuresJson: "" },
  });
  ...
  const onSubmit = async (data: ProductFormInput) => {
    ...
    try {
      ...
      if (fieldErrors) {
        ...
        showToast(
          err instanceof ApiError ? err.message : "Oluşturma başarısız",
          "error",
        );
      }
    }
  ...
  if (categoriesLoading) return <Skeleton className="h-96 w-full" />;
